...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Project a verified GameSnapshot into a wot-gui-assets data tree."
    )
...
def _write_github_result(result: dict[str, object]) -> None:
    ...
    if summary_path:
        ...
        with open(summary_path, "a", encoding="utf-8") as summary:
            summary.write("## wot-gui-assets publication\n\n")
...
def _write_failure(error: PublicationError) -> None:
    ...
    if os.environ.get("GITHUB_ACTIONS") == "true":
        ...
        print(
            f"::error title=wot-gui-assets publication failed::{escaped}",
            file=sys.stderr,
            flush=True,
        )
