...
def _assert_no_collisions(paths: list[str]) -> None:
    ...
    for folded, path in seen.items():
        ...
        for index in range(1, len(parts)):
            ...
            if prefix in seen:
                raise PublicationError(
                    f"file/directory output collision: {seen[prefix]!r} and {path!r}"
                )


def _commit_subject(files: tuple[PayloadFile, ...]) -> str:
    candidates = [
        item
        for item in files
        if item.language is None and item.path.casefold() == "version.xml"
    ]
    if len(candidates) != 1:
        raise PublicationError("snapshot must contain exactly one base root version.xml")
    try:
...
Перед version commit большой публикации в истории могут находиться служебные staging commits.
...
def project_snapshot(
    ...
    *,
    ...
    try:
        projected_file_count = len(assets) + sum(
            len(items) for items in locale_assets.values()
        )
        ...
        output_paths = ["README.md", ".version_name", ".publication.json", *assets]
        output_paths.extend(
            f"locales/{language}/{path}"
            for language, items in locale_assets.items()
            for path in items
        )
        _assert_no_collisions(output_paths)

        for path, item in sorted(assets.items()):
            _copy(item, temporary, path)
            projection_progress.advance(item.size)
        for language, items in locale_assets.items():
            for path, item in sorted(items.items()):
                _copy(item, temporary, f"locales/{language}/{path}")
                projection_progress.advance(item.size)

        release_name = _string(source.get("release_name"), label="snapshot release name")
        commit_subject = _commit_subject(snapshot.files)
        ...
        publication: dict[str, Any] = {
            ...
            "counts": {
                "assets": len(assets),
                "locales": {language: len(items) for language, items in locale_assets.items()},
            },
...
def publish_snapshot(
    ...
    *,
    ...
    temporary = Path(tempfile.mkdtemp(prefix="wot-gui-assets-publication-"))
