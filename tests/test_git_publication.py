...
def test_github_api_finalization_creates_commit_and_fast_forward_ref(
    ...
    monkeypatch.setenv("PUBLISHER_GITHUB_REPOSITORY", "wotstat/wot-gui-assets")
    ...
    def fake_api_request(
        ...
        **_kwargs: object,
        ...
        assert api_repository == "wotstat/wot-gui-assets"
...
def _legacy_bootstrap_readme() -> str:
    ...
    previous = current.replace(
        "Каждая\nпубликация завершается version commit: его сообщение строится из корневого "  # noqa: RUF001
        "`version.xml` snapshot\nбез префикса `v.` в формате `2.3.1.0 #903`, а точный release "  # noqa: RUF001
